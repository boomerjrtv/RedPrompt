...
(async () => {
  const flagSets = [
    { name: 'default-headless-new', args: ['--enable-unsafe-webgpu', '--no-sandbox'] },
    { name: 'vulkan', args: ['--enable-unsafe-webgpu', '--enable-features=Vulkan,UseSkiaRenderer', '--use-angle=vulkan', '--no-sandbox'] },
    { name: 'gl-egl', args: ['--enable-unsafe-webgpu', '--use-gl=egl', '--no-sandbox'] },
    { name: 'swiftshader', args: ['--enable-unsafe-webgpu', '--use-angle=swiftshader', '--no-sandbox'] },
  ];
  for (const fs of flagSets) {
    let browser;
    try {
      browser = await chromium.launch({ headless: true, args: fs.args });
